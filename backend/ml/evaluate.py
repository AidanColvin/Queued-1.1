...
The same holdout is scored in three item-vector spaces so improvements are
apples-to-apples, plus a non-personalized popularity baseline and random:
  * ``semantic`` — MiniLM plot-summary embeddings (what production uses today).
  * ``cf``       — collaborative-filtering item factors (SVD on ratings).
  * ``hybrid``   — 50/50 blend of the two (per-row unit-norm, concatenated).
...
from ml.reranker import SessionReranker, _unit_rows, build_taste_space

LIKE_AT = 4.0
DISLIKE_AT = 2.0
NEUTRAL_MS = 3000  # ~neutral deliberation -> time_modifier == 1.0
SPACES = ("semantic", "cf", "hybrid")

ARTIFACTS = Path("data/artifacts")


def _load() -> tuple[dict[str, np.ndarray], dict[int, int], pd.DataFrame]:
    """Return ({space: item-matrix}, movie_id->idx, catalog ratings)."""
    ...
    # `hybrid` uses the SAME builder production ships, so the measured number is
    # exactly what runs live.
    hybrid = build_taste_space(raw_emb, raw_cf)
    matrices = {"semantic": semantic, "cf": cf, "hybrid": hybrid}
    ...
    return matrices, movieid_to_idx, ratings
...
def evaluate(n_users: int, min_ratings: int, holdout_frac: float, seed: int) -> dict:
    matrices, movieid_to_idx, ratings = _load()
    ...
    # Pooled (score, label) per space, plus popularity; per-user precision@k.
    pooled = {s: [] for s in SPACES} | {"popularity": []}
    labels_all: list[int] = []
    p_at = {s: {5: [], 10: []} for s in (*SPACES, "popularity")}
    n_eval_users = 0
    ...
    for _uid, grp in sample.groupby("userId", sort=False):
        ...
        for space in SPACES:
            sc = space_scores[space]
            pooled[space].extend((sc if sc is not None else np.zeros(labels.size)).tolist())
        ...
        for k in (5, 10):
            if labels.size >= k:
                for space in SPACES:
                    sc = space_scores[space]
                    if sc is not None:
                        p_at[space][k].append(labels[np.argsort(-sc)[:k]].mean())
...
def main() -> None:
    ...
    print(f"{'ranking space':<14}{'ROC-AUC':>10}{'P@5':>9}{'P@10':>9}")
    print("-" * 42)
    label_for = {"semantic": "semantic*", "cf": "cf", "hybrid": "hybrid", "popularity": "popularity"}
    for col in (*SPACES, "popularity"):
        m = r["rows"][col]
        print(f"{label_for[col]:<14}{m['auc']:>10.3f}{m['p@5']:>9.3f}{m['p@10']:>9.3f}")
    print(f"{'random':<14}{0.5:>10.3f}{base:>9.3f}{base:>9.3f}")
    print("\n* = what production uses today.  AUC = P(liked title outranks disliked). "
